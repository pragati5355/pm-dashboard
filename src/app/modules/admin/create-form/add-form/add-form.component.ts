...
export class AddFormComponent implements OnInit {
    ...
    activeCustomers = [
        {
            ...
            labelPosition: 'top',
            description: '',
            tooltip: 'fjlkdjf fsdaflkj',
            customClass: 'inputclasstest',
            tabindex: '',
            hidden: false,
            hideLabel: false,
            autofocus: false,
            ...
            modalEdit: false,
            questions: [
                { label: 'sans', value: 'sans', tooltip: '' },
                { label: 'vis', value: 'vis', tooltip: '' },
            ],
            values: [
    ...
    addForm() {
        const json = document.createTextNode(
            JSON.stringify(this.formio.form, null, 4)
        );
        let payload = {
    ...
    drop(event: CdkDragDrop<any[]>) {
        let data = {
            ...
            tableView: false,
            modalEdit: false,
            questions: this.teammemberQuestion,
            values: [
                { label: 'Excellent', value: 5, tooltip: '' },
        ...
        this.ProjectService.getTeamMemberList(payload).subscribe(
            (res: any) => {
                ...
                if (this.form.components.length > 1) {
                    let surveyCompoenent: any[] = this.form.components.filter(
                        (item: any) => item.customClass == 'surveyCopmonent'
                    );
