...
export class ProfitLossProjectStatisticComponent implements OnInit {
    ...
    displayedResoureCostColumns: string[] = [
        ...
        'idealworkhours',
        'hoursasperprojectplan',
        'actualworkloghrs',
        'hourlycostonproject',
        'costasperprojectplan',
        'actualcostonproject',
        'costdifference',
    ];
    routeSubscribe: any;
    ...
    loadStatList() {
        ...
        if (
            this.datePipe.transform(this.range?.value?.endDate, 'yyyy-MM-dd') !=
            null
        ) {
            this.pNLProjectServie.getPNLStatList(this.payload).subscribe(
                (res: any) => {
                    ...
                    if (res?.statusCode === 200) {
                        ...
                        if (
                            this.projectStatDetails?.resourceCost?.length != 0
                        ) {
    ...
    private getCurrentMonthAndYear() {
        ...
        this.currentMonthCurrentDate = new Date(
            date.getFullYear(),
            date.getMonth(),
            date.getDate()
        );
